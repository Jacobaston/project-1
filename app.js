...
function moveGhost(ghost) {
  const movemennt = [+1, + width, -1, - width]
  let ghostimerId = ''
  let direction = movemennt[Math.floor(Math.random() * movemennt.length)]

  if (direction === width) {
    direction = movemennt[Math.floor(Math.random() * movemennt.length)]
  ...
  ghostimerId = setInterval(function () {
    if (!cells[ghost + direction].classList.contains('wall')) {
      ...
      cells[ghost].classList.add('ghost')
    } else direction = movemennt[Math.floor(Math.random() * movemennt.length)]

    if (cells[ghost].classList.contains('pacmanRight')) {
      alert('Game Over!')
